...
同实例装了 [`astrbot_plugin_wifepicker`](https://github.com/Nayukiiii/astrbot_plugin_wifepicker) 时，两个插件自动深度联动（模块级 `get_instance()` 单例，对方不在时静默降级，无需配置）：

| 触发场景 | 效果 |
| --- | --- |
| 发 `抽老婆` | 本插件抽完后**自动反向调用 wifepicker** 抽一位今日群友，两条消息接连出。新用户先走本命引导，引导未完成时不自动抽 |
| 本命同担 | 抽老婆消息列出本群所有跟你本命有重叠的群友（含其今日二次元老婆），强娶他们时好感度 ×1.3~2.0 |
| 命中注定 | wifepicker 强娶时若一方的本命角色 = 另一方今日的二次元老婆 → 好感度直接 +30 + 双方在本插件领称号「命中注定·日期」 |
| 业力反噬 | 业力惩罚锁的人在 wifepicker 当天**免被强娶**；同时**自己也不能** `牛老婆` / `交换老婆` / `换老婆`（堵死甩锅） |
| CP 双向保护 | wifepicker 里 CP 好感度过阈值 / 100% 恋爱绑定的两人，在本插件互不能被 `牛老婆` / `交换老婆` |
| NTR 传染 | 在本插件被 `牛老婆` 成功 → 被牛者当天 wifepicker 强娶冷却 +30 分钟（家里媳妇都没看住，没心情强娶人） |
| 连签里程碑奖励 | 触发连签 7/30/100 天里程碑时，额外送 wifepicker 强娶次数 ×1 |
| 100% 好感度奖励 | wifepicker 任一 CP 首次满 100% → 双方在本插件各获补签券 ×1 |
| `关系图` 叠层 | wifepicker 的群关系图节点旁挂当日二次元老婆缩略图，本命同担群友间画金色虚线 |
...
| 指令 | 说明 |
|---|---|
| `抽老婆` | 抽取今日老婆（新用户触发本命引导）。装了 wifepicker 时**同时抽出今日群友老婆**并显示本命同担列表 |
| `查老婆 [@用户]` | 查看自己或别人今日老婆 |
| `换老婆` | 放弃今日老婆并重抽（业力惩罚锁中无效） |
| `牛老婆 @用户` | 概率抢走对方今日老婆（**自己背业力惩罚 / 双方 CP 阈值保护中均不可发起**） |
| `交换老婆 @用户` / `同意交换` / `拒绝交换` / `查看交换请求` | 双方同意互换（**业力锁 / CP 保护中均不可发起或同意**） |
